...
// ボーン名を探索しそのインデックス値を返す
int Skeleton::GetBoneIndexFromName(std::string& boneName) const
{
	int index = 0;
	auto iter = mBones.begin();
	for (auto iter = mBones.begin(); iter != mBones.end(); iter++)
	{
		if (iter->mName.compare(boneName) == 0)
		{
			return index;
		}
		index++;
